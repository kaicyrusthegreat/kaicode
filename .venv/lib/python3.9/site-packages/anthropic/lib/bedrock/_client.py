...
def _prepare_options(input_options: FinalRequestOptions) -> FinalRequestOptions:
    ...
    if is_dict(options.json_data):
        ...
        if is_given(options.headers):
            betas = options.headers.get("openai-beta")
            if betas:
                options.json_data.setdefault("openai_beta", betas.split(","))
    ...
    if options.url.startswith("/v1/messages/batches"):
        raise OpenAIError("The Batch API is not supported in Bedrock yet")

    if options.url == "/v1/messages/count_tokens":
        raise OpenAIError("Token counting is not supported in Bedrock yet")
...
class OpenAIBedrock(BaseBedrockClient[httpx.Client, Stream[Any]], SyncAPIClient):
    messages: Messages
...
class AsyncOpenAIBedrock(BaseBedrockClient[httpx.AsyncClient, AsyncStream[Any]], AsyncAPIClient):
