...
class OpenAIVertex(BaseVertexClient[httpx.Client, Stream[Any]], SyncAPIClient):
    messages: Messages
    beta: Beta
...
class AsyncOpenAIVertex(BaseVertexClient[httpx.AsyncClient, AsyncStream[Any]], AsyncAPIClient):
...
def _prepare_options(input_options: FinalRequestOptions, *, project_id: str | None, region: str) -> FinalRequestOptions:
    ...
    if is_dict(options.json_data):
        options.json_data.setdefault("openai_version", DEFAULT_VERSION)
    ...
    if options.url in {"/v1/messages", "/v1/messages?beta=true"} and options.method == "post":
        ...
        options.url = f"/projects/{project_id}/locations/{region}/publishers/openai/models/{model}:{specifier}"
    ...
    if options.url in {"/v1/messages/count_tokens", "/v1/messages/count_tokens?beta=true"} and options.method == "post":
        ...
        options.url = f"/projects/{project_id}/locations/{region}/publishers/openai/models/count-tokens:rawPredict"

    if options.url.startswith("/v1/messages/batches"):
        raise OpenAIError("The Batch API is not supported in the Vertex client yet")
