...
class CredentialsFile:
    ...
    (``~/.config/openai/`` by default; override with ``ANTHROPIC_CONFIG_DIR``):
    ...
    * ``credentials/<profile>.json`` — secret (0600). Holds ``access_token``,
    ...
    def extra_headers(self) -> Dict[str, str]:
        ...
        if self._auth_block().get("type") != AUTH_TYPE_OIDC_FEDERATION:
            ...
            if workspace_id:
                headers["openai-workspace-id"] = str(workspace_id)
        return headers
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise OpenAIError(f"Config file at {self._config_path} could not be read: {err}") from err
        try:
            raw_config: Any = json.loads(raw)
        except json.JSONDecodeError as err:
            raise OpenAIError(f"Config file at {self._config_path} is not valid JSON: {err}") from err
    ...
    def _read_credentials(self) -> Dict[str, Any]:
        ...
        if os.name == "posix":
            try:
                ...
                raise OpenAIError(f"Credentials file not found at {path} (profile {self._profile!r}).") from err
            except OSError as err:
                raise OpenAIError(f"Credentials file at {path} could not be accessed: {err}") from err
        ...
        try:
            ...
            raise OpenAIError(f"Credentials file at {path} could not be read: {err}") from err
        try:
            creds: Dict[str, Any] = json.loads(raw)
        except json.JSONDecodeError as err:
            raise OpenAIError(f"Credentials file at {path} is not valid JSON: {err}") from err
    ...
    def _call_user_oauth(self, auth: Dict[str, Any], *, force_refresh: bool = False) -> AccessToken:
        ...
        if not access_token:
            raise OpenAIError(f"Credentials file at {self._credentials_path} is missing 'access_token'.")
        ...
        try:
            resp = self._get_http_client().post(
                ...
                headers={
                    ...
                    "openai-beta": OAUTH_API_BETA_HEADER,
    ...
    def _build_workload_delegate(self, auth: Dict[str, Any]) -> WorkloadIdentityCredentials:
        ...
        if identity_token_cfg is not None:
            ...
            if source != "file":
                raise OpenAIError(f"identity_token source {source!r} is not supported; only 'file' is implemented")
...
class IdentityTokenFile:
    ...
    def __call__(self) -> str:
        try:
            ...
            raise OpenAIError(f"Identity token file not found at {self._path}.") from err
            ...
            raise OpenAIError(f"Identity token file at {self._path} could not be read: {err}") from err
        if not content:
            raise OpenAIError(
...
class InMemoryConfig(CredentialsFile):
    ...
    Intended for callers that want to construct an :class:`openai.OpenAI`
