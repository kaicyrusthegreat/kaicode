...
def _user_agent() -> str:  # pyright: ignore[reportUnusedFunction] — used by _workload/_providers
    ...
    return f"openai-python/{__version__}"
...
def _config_dir() -> pathlib.Path:
    ...
    Platform defaults:
      * Linux & macOS: ``~/.config/openai/`` — XDG-style on both platforms
        for consistency across SDKs (macOS does **not** use
        ``~/Library/Application Support/``).
      * Windows: ``%APPDATA%\\OpenAI\\``
    """
    ...
    if sys.platform == "win32":
        ...
        return base / "OpenAI"
    return pathlib.Path.home() / ".config" / "openai"
...
def _validate_profile_name(profile: str, *, source: str = "profile name") -> None:
    ...
    """
    if not profile:
        raise OpenAIError(f"{source} must not be empty.")
    if profile != profile.strip():
        raise OpenAIError(f"{source} {profile!r} has leading or trailing whitespace.")
    if profile.startswith("."):
        raise OpenAIError(f"{source} {profile!r} must not start with a dot.")
    for sep in ("/", "\\", os.sep):
    ...
    if "\x00" in profile:
        raise OpenAIError(f"{source} {profile!r} must not contain null bytes.")
...
def _resolve_under(base: pathlib.Path, candidate: pathlib.Path) -> pathlib.Path:
    ...
    try:
        ...
        raise OpenAIError(f"Resolved path {candidate_resolved} escapes config directory {base_resolved}.") from err
    return candidate
...
def _has_auto_discoverable_credentials() -> bool:  # pyright: ignore[reportUnusedFunction] — used by _client
    ...
    static ``OPENAI_API_KEY`` / ``ANTHROPIC_AUTH_TOKEN`` is set alongside
