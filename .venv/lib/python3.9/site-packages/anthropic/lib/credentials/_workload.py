...
class WorkloadIdentityError(OpenAIError):
...
class WorkloadIdentityCredentials:
    """Exchanges an external OIDC JWT for an OpenAI access token via the
    ...
    ``credentials=`` to :class:`openai.OpenAI`) to avoid exchanging on every
    request.
    ...
    Args:
        ...
        workspace_id: Optional ``wrkspc_*`` tagged ID, or the literal
            ...
            selection (the ``openai-workspace-id`` header) is not supported
    ...
    def bind_base_url(self, base_url: str) -> None:
        ...
        Called by :class:`openai.OpenAI` when this object is passed as
    ...
    def __call__(self, *, force_refresh: bool = False) -> AccessToken:
        ...
        try:
            resp = self._http_client.post(
                ...
                json=body,
                headers={
                    "openai-beta": _JWT_BEARER_BETA_HEADER,
        ...
        if resp.status_code >= 400:
            ...
            if resp.status_code == 401:
                ...
                hint += (
                    "View your authentication events in the Workload identity page of AI Console for more details."
                )
...
def exchange_federation_assertion(
    *,
    ...
    OpenAI access token back (no caching, no provider plumbing).
    """
