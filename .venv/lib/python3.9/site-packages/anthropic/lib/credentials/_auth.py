...
def warn_env_static_shadows_auto_discovery(env_var: str) -> None:
    """Warn that an ``OPENAI_API_KEY`` / ``ANTHROPIC_AUTH_TOKEN`` from the
...
class AccessTokenAuth(httpx.Auth):
    ...
    Used by :meth:`openai.OpenAI.custom_auth` to inject ``Authorization: Bearer``
    ...
    a static ``OPENAI_API_KEY`` shadows any credentials provider.
    """
    ...
    def _apply(self, request: httpx.Request, token: str) -> None:
        ...
        existing_beta = request.headers.get("openai-beta", "")
        ...
        if OAUTH_API_BETA_HEADER not in existing_flags:
            ...
            request.headers["openai-beta"] = ", ".join(existing_flags)
