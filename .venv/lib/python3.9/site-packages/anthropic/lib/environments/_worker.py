...
:class:`~openai.lib.tools.agent_toolset.AgentToolContext` and downloads the
session agent's skills, then runs a
:class:`~openai.lib.tools._beta_session_runner.SessionToolRunner` for the
...
or construct it directly::
    ...
    from openai.lib.environments import EnvironmentWorker
...
# ``EnvironmentWorker`` without dragging those imports into ``import openai``.
    ...
    *while* heartbeating the work-item lease in parallel; on exit it force-stops
    ...
    Args:
      ...
      max_idle: Forwarded to the session tool runner — seconds to keep running
        ...
        to :data:`~openai.lib.environments.DEFAULT_MAX_IDLE` (60s). ``None``
        disables it.
