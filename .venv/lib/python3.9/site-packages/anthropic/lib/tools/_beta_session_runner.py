...
:class:`openai.lib.environments.EnvironmentWorker` if you need heartbeating /
force-stop.
"""
...
__all__ = [
    ...
    "MANAGED_AGENTS_BETA",
    "DEFAULT_MAX_IDLE",
    # Re-exported for ``openai.lib.environments._worker``, which drives the
...
# openai-beta gating Sessions access to self-hosted environments. The Sessions
...
def _scoped_client(client: AsyncOpenAI, environment_key: str | None) -> AsyncOpenAI:
...
class SessionToolRunner:
    ...
    wrap it in an
    :class:`~openai.lib.environments.EnvironmentWorker` for heartbeating /
    force-stop.
...
async def _run_session_tools(
    ...
    *,
    ...
    :class:`~openai.lib.environments.EnvironmentWorker` enters the runner
