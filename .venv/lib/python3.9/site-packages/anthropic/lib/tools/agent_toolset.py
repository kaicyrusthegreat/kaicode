...
``openai.lib.tools.__init__`` — depend on it explicitly
(``from openai.lib.tools.agent_toolset import beta_agent_toolset_20260401``).
...
:class:`~openai.lib.environments.EnvironmentWorker` — the self-hosted
environment worker. The sync ``OpenAI`` ``messages.tool_runner`` accepts
...
log = logging.getLogger("openai.lib.tools.agent_toolset")
...
def _default_bash_env() -> dict[str, str]:
    ...
    session tokens): a prompt-injected ``echo $OPENAI_API_KEY`` would
...
def beta_agent_toolset_20260401(ctx: AgentToolContext) -> list[BetaAsyncFunctionTool[Any]]:
    ...
    The result is a plain list of :class:`~openai.lib.tools.BetaAsyncFunctionTool`
    instances — *async* function tools, so it is for the **async** runners only:
    the ``AsyncOpenAI`` ``client.beta.messages.tool_runner`` and
    ``client.beta.sessions.events.tool_runner`` (always async). The sync
    ``OpenAI`` ``messages.tool_runner`` takes ``BetaRunnableTool``, which
