...
def _is_base_client(client: object) -> bool:
    """True only for the base ``OpenAI`` / ``AsyncOpenAI`` classes, not subclasses.

    Subclasses (``OpenAIAWS``, ``OpenAIFoundry``) have their own auth paths
    and must not run the credential chain or forward ``credentials`` through their
    ``__init__`` (which doesn't accept the kwarg).
    """
    return type(client) in (OpenAI, AsyncOpenAI)
...
def _warn_env_shadow(*, api_key: str | None, auth_token: str | None) -> None:
    """Warn when an ``OPENAI_API_KEY`` / ``ANTHROPIC_AUTH_TOKEN`` from the
    ...
    if not _has_auto_discoverable_credentials():
        return
    if api_key is not None and os.environ.get("OPENAI_API_KEY"):
        warn_env_static_shadows_auto_discovery("OPENAI_API_KEY")
...
class OpenAI(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        2. ``OPENAI_API_KEY`` / ``ANTHROPIC_AUTH_TOKEN`` environment
           variables.
        ...
        ``OPENAI_API_KEY`` / ``ANTHROPIC_AUTH_TOKEN`` is set alongside a
        profile or federation configuration, the static credential takes
        precedence and a one-shot warning is logged on the ``openai``
        logger.
        """
        # --- credentials support (hand-written, upstream to Stainless) ---
        ...
        if base_url is None:
            base_url = f"https://api.openai.com"
        ...
        if credentials is None and api_key is None and auth_token is None and _is_base_client(self):
            result = default_credentials(base_url=str(base_url) if base_url else "https://api.openai.com")
