...
def print_plan(content: str) -> None:
    ...
    renderable = (
        ...
        else Text(content, style="kaicode.msg.plan")
    )
    console.print()
    console.print(
        Panel(
...
def print_kai_message(content: str) -> None:
    renderable = (
        Markdown(content, code_theme="monokai")
        if ("```" in content or "`" in content)
        else Text(content, style="kaicode.msg.kai")
    )
    console.print()
    console.print(
        Panel(
...
def print_tool_result(tool_name: str, result: str) -> None:
    ...
    if tool_name == "grep_ast" and "results" in data:
        ...
        if not results:
            console.print(
                Text(f"  No definitions found for '{data.get('symbol','')}'", style="kaicode.muted")
            )
            return
        ...
        for r in results[:20]:
            table.add_row(
                ...
                str(r["line"]),
                r.get("type", ""),
                r.get("signature") or r.get("content", ""),
            )
        console.print(table)
        return
...
def _print_diff(diff: str, title: str = "diff", border: str = "kaicode.separator") -> None:
    if not diff:
    ...
    for line in diff.splitlines():
        if line.startswith("+") and not line.startswith("+++"):
            ...
            text.append(line + "\n", style="kaicode.dir")
    console.print(
        Panel(text, title=f"[kaicode.dir]{title}[/]", border_style=border, padding=(0, 1))
    )
...
def _emit_diff_line(
    out: Text, lineno: str, marker: str, content: str, bg: str, marker_style: str, width: int
) -> None:
...
def _render_diff_body(diff: str) -> Text:
    ...
    for raw in diff.splitlines():
        ...
        if raw.startswith("@@"):
            ...
            continue
        if raw.startswith("+"):
            _emit_diff_line(out, str(new_ln), "+ ", raw[1:], _DIFF_ADD_BG, "bold green", width)
            new_ln += 1
        elif raw.startswith("-"):
            _emit_diff_line(out, str(old_ln), "- ", raw[1:], _DIFF_DEL_BG, "bold red", width)
            old_ln += 1
        else:
...
def print_change_preview(tool_name: str, args: dict, cwd: str) -> bool:
    ...
    if tool_name == "create_file":
        ...
        _print_change_header("Overwrite" if exists else "Create", path, len(lines), 0)
        width = _diff_width()
        out = Text()
        for i, line in enumerate(lines[:200], 1):
            _emit_diff_line(out, str(i), "+ ", line, _DIFF_ADD_BG, "bold green", width)
        if len(lines) > 200:
...
def _print_search_results(data: dict) -> None:
    ...
    if data.get("truncated"):
        console.print(
            Text(f"  (showing first 30 of {data['total']} results)", style="kaicode.muted")
        )
    console.print(table)
...
def _print_git_status(data: dict) -> None:
    ...
    for f in data.get("untracked", []):
        console.print(Text(f"  ? {f}", style="kaicode.muted"))
    if data.get("clean"):
...
def print_help() -> None:
    ...
    console.print(
        Text(
            "  ·  @path        include a specific file as context  (e.g. @kaicode/app.py)",
            style="kaicode.muted",
        )
    )
    console.print(
    ...
    console.print(
        Text("  ·  /undo        safely revert the agent's last file change", style="kaicode.muted")
    )
    console.print(
        Text(
            "  ·  KAICODE.md   standing instructions, auto-loaded every session",
            style="kaicode.muted",
        )
    )
    console.print(
        Text(
            "  ·  Read-only tools run without asking; writes/exec ask first", style="kaicode.muted"
        )
    )
    console.print()
